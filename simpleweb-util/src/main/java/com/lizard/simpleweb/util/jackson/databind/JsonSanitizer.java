package com.lizard.simpleweb.util.jackson.databind;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Given JSON-like content, converts it to valid JSON. This can be attached at either end of a
 * data-pipeline to help satisfy Postel's principle:
 *
 * <blockquote>
 * <p>
 ...
 * </blockquote>
 *
 * <p>Applied to JSON-like content from others, it will produce well-formed JSON that should satisfy
 * any parser you use.
 *
 * <p>Applied to your output before you send, it will coerce minor mistakes in encoding and make it
 * easier to embed your JSON in HTML and XML.
 *
 * <h2>Input</h2>
 * <p>
 * The sanitizer takes JSON like content, and interprets it as JS eval would. Specifically, it deals
 * with these non-standard constructs.
 *
 * <ul>
 ...
 * </ul>
 * <p>
 * The sanitizer fixes missing punctuation, end quotes, and mismatched or missing close brackets. If
 * an input contains only white-space then the valid JSON string {@code null} is substituted.
 *
 * <h2>Output</h2>
 * <p>
 * The output is well-formed JSON as defined by <a href="http://www.ietf.org/rfc/rfc4627.txt">RFC
 * 4627</a>. The output satisfies three additional properties:
 *
 * <ol>
 *   <li>The output will not contain the substring (case-insensitively) {@code "</script"} so can be
 *       embedded inside an HTML script element without further encoding.
 *   <li>The output will not contain the substring {@code "]]>"} so can be embedded inside an XML
 *       CDATA section without further encoding.
 *   <li>The output is a valid Javascript expression, so can be parsed by Javascript's <code>eval
 *       </code> builtin (after being wrapped in parentheses) or by <code>JSON.parse</code>.
 *       Specifically, the output will not contain any string literals with embedded JS newlines
 *       (U+2028 Paragraph separator or U+2029 Line separator).
 *   <li>The output contains only valid Unicode scalar values (no isolated UTF-16 surrogates) that
 *       are <a href="http://www.w3.org/TR/xml/#charsets">allowed in XML</a> unescaped.
 * </ol>
 *
 * <h2>Security</h2>
 * <p>
 * Since the output is well-formed JSON, passing it to <code>eval</code> will have no side-effects
 * and no free variables, so is neither a code-injection vector, nor a vector for exfiltration of
 * secrets.
 *
 * <p>This library only ensures that the JSON string &rarr; Javascript object phase has no side
 * effects and resolves no free variables, and cannot control how other client side code later
 * interprets the resulting Javascript object. So if client-side code takes a part of the parsed
 * data that is controlled by an attacker and passes it back through a powerful interpreter like
 * {@code eval} or {@code innerHTML} then that client-side code might suffer unintended
 * side-effects.
 *
 * <h2>Efficiency</h2>
 * <p>
 * The sanitize method will return the input string without allocating a new buffer when the input
 * is already valid JSON that satisfies the properties above. Thus, if used on input that is usually
 * well formed, it has minimal memory overhead.
 *
 * <p>The sanitize method takes O(n) time where n is the length in UTF-16 code-units.
 */
public final class JsonSanitizer {
    ...
    /**
     * Given JSON-like content, produces a string of JSON that is safe to embed, safe to pass to
     * JavaScript's {@code eval} operator.
     *
    ...
    /**
     * Same as {@link JsonSanitizer#sanitize(String)}, but allows to set a custom maximum nesting
     * depth.
     *
     * @param jsonish             JSON-like content.
    ...
    /**
     * Describes where we are in a state machine that consists of transitions on complete values,
     * colons, commas, and brackets.
     */
    private enum State {
        /**
        ...
        BEFORE_ELEMENT,
        /**
         * After a JSON value in an array or at the top level, and before any following comma or
         * close bracket.
         */
        AFTER_ELEMENT,
        /**
    ...
    /**
     * The number of brackets that have been entered and not subsequently exited. Also, the length
     * of the used prefix of {@link #isMap}.
     */
    private int bracketDepth;
    /**
     * {@code isMap[i]} when {@code 0 <= i && i < bracketDepth} is true iff the i-th open bracket
     * was a <code>'{'</code>, not a <code>'['</code>.
     */
    private boolean[] isMap;
    /**
     * If non-null, then contains the sanitized form of {@code jsonish.substring(0, cleaned)}. If
     * {@code null}, then no unclean constructs have been found in {@code jsonish} yet.
     */
    private StringBuilder sanitizedJson;
    /**
     * The length of the prefix of {@link #jsonish} that has been written onto {@link
     * #sanitizedJson}.
     */
    private int cleaned;
            ...
            try {
                ...
                switch (ch) {
                    ...
                    case ']':
                        ...
                        state =
                                bracketDepth == 0 || !isMap[bracketDepth - 1]
                                        ? State.AFTER_ELEMENT
                                        : State.AFTER_VALUE;
                        break;
                    case ',':
    ...
    /**
     * Ensures that the output corresponding to {@code jsonish[start:end]} is a valid JSON string
     * that has the same meaning when parsed by Javascript {@code eval}.
     *
     * <ul>
     *   <li>Making sure that it is fully quoted with double-quotes.
     *   <li>Escaping any Javascript newlines : CR, LF, U+2028, U+2029
     *   <li>Escaping HTML special characters to allow it to be safely embedded in HTML {@code
     *       <script>} elements and XML {@code <!CDATA[...]]>} sections.
    ...
    /**
     * The position past the last character within the quotes of the quoted string starting at
     * {@code s.charAt(start)}. Does not assume that the quoted string is properly closed.
     */
        ...
        for (int i = sanitizedJson.length(); --i >= 0; ) {
            switch (sanitizedJson.charAt(i)) {
                ...
                case ',':
                    ...
                    return;
                default:
                    throw new AssertionError("" + sanitizedJson.charAt(i));
            }
        }
     ...
     * {@link #canonicalizeNumber} since it can be called on inputs that are valid JSON so is on the
     * fast path.
     *
     * <p>JS numbers differ from JSON numbers in several ways:
     *
     * <ul>
    ...
    /**
     * Converts a run of characters that form a JS number to its canonical form which happens to
     * also be a valid JSON number.
     *
